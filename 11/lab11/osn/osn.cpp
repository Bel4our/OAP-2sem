...
int sum(Tree* t); // сумма вершин
int evenTop(Tree* t); // колиество чётных вершин
int sumLeav(Tree* t); // сумма листьев дереа
int countLeav(Tree* p1); // количество листьев дерева
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	подсчёт суммы вершин\n";
		cout << "9 -	подсчёт количества чётных ключей вершин\n";
		cout << "10 -	подсчёт суммы листьев дерева\n";
		cout << "11 -	подсчёт количества листьев дерева\n";
		cout << "12 -	выход\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		...
		case 8: cout << "Сумма всех вершин дерева "<<sum(Root) << endl; break; // передаём корень нашего дерева
		case 9: cout << "Количество чётных ключей вершин " << evenTop(Root) << endl; break; 
		case 10: cout << "Сумма всех листьев дерева " << sumLeav(Root) << endl; break;
		case 11:  cout << "Количество листьев дерева " << countLeav(Root) << endl; break;
		case 12:  exit(0);
		}
	}
}
...
int sum(Tree* p1)  // функция для подсчёта суммы вершин дерева (основной вариант)
{
	if (p1 == nullptr) 
	{
		return 0; 
...
int evenTop(Tree* p1) // функция для определения количества узлов с четными ключами (доп вариант 12)
{
	if (p1 == nullptr)
	{
		return 0;
...
int sumLeav(Tree* p1) // функция для определения суммы листьев (доп вариант 16)
{
	if (p1 == nullptr)
	{
		return 0; 
	}
	else if (p1->Left == nullptr && p1->Right == nullptr) // проверка узла на то является он листом или нет?
	{
		return p1->key; 
	}
	else
...
int countLeav(Tree* p1) // функция для определения количества листьев (доп вариант 7)
{
	if (p1 == nullptr)
	{
		return 0;
