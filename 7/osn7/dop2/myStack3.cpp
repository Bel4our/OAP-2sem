...
void find(Stack*& myStk)  // функция поиска и удаления первого положительного элемента в стеке
{
	bool f = true; // для ограничения поиска чисел
	Stack* prev = NULL;
	Stack* e = myStk;
	while (e != NULL)
	{
		if (e->data > 0 && f)
		{
			if (prev == NULL) // если положительный элемент - первый в стеке
			{
				myStk = e->next;
			}
			else
	...
	if (f)
	{
		cout << endl << "Положительных элементов не найдено" << endl; // Если ни один элемент не нашелся 
	}
}