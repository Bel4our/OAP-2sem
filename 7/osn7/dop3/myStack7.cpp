...
void find(Stack*& myStk)  // функция поиска и удаления первого элемента больше 100 в стеке
{
	...
	while (e != NULL)
	{
		if (e->data > 100 && f)
		{
			if (prev == NULL) // если нужный элемент - первый в стеке
			{
				myStk = e->next;
			}
			else
	...
	if (f)
	{
		cout << endl << "нужных элементов не найдено" << endl; // Если ни один элемент не нашелся 
	}
}