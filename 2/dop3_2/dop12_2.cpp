...
int main()
{
    ...
    // создаём массивы для хранения чисел из файла
    int numb1[1000];
    int numb2[1000];
    int numb3[1000];
    int i = 0;
    // записываем числа из файла в массивы
    ...
    for (int j = 0; j < i; j++)
    {
        if (numb1[j] <= numb2[j] && numb1[j] <= numb3[j]) // если число из первого массива меньше
        {
        ...
        {
            if (numb2[j] <= numb1[j] && numb2[j] <= numb3[j])  // если число из второго массива меньше
            {
                fprintf(f4, "%d ", numb2[j]); // записываем его в файл
                printf("%d ", numb2[j]); // выводим в консоль
            }
            else
            {
                fprintf(f4, "%d ", numb3[j]); // записываем число из 3 массива в файл
                printf("%d ", numb3[j]); // выводим в консоль
            }
        }
    }
    //закрываем файлы
