...
void input(list*&, float); //функция добавления элемента, передается адрес списка и символ, который добавляется 
...
void proizv10(list*); //функция подсчета произведения чисел, меньших 10  
...
int main()
{
    ...
    do 
    {
        ...
        case 6:
            ...
            break;
        case 7:
            proizv10(first);	// вычисление произведения
            break;
        }
    } while (choice!=8);      
    return 0;
...
void input(list*& first, float value) //Добавление числа value в список 
{
...
void proizv10(list* p)  // Подсчет произведения чисел, меньших 10  
{
    float pr = 1;
    if (p == NULL)
        cout << "Список пуст" << endl;
